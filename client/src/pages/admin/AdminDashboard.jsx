...
const StatCard = ({ title, value, icon: Icon, accent, subtitle, loading }) => (
  <div style={{
    ...
    <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
      <div style={{ flex: 1 }}>
        <p style={{ fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color: loading ? `${accent}60` : accent, marginBottom: 4 }}>
          {title}
        </p>
        {loading ? (
          <div style={{ marginTop: 12 }}>
            <div style={{ ...skeletonBase, height: 34, width: "55%", marginBottom: 10, borderRadius: 8 }} />
            <div style={{ ...skeletonBase, height: 10, width: "75%" }} />
          </div>
        ) : (
          <>
            <p style={{ fontSize: "2rem", fontWeight: 900, color: "#fafafa", lineHeight: 1.1, marginTop: 6 }}>{value}</p>
            {subtitle && <p style={{ fontSize: "0.72rem", marginTop: 4, color: "#71717a" }}>{subtitle}</p>}
          </>
        )}
      </div>
      <div style={{ background: `${accent}${loading ? "0d" : "18"}`, borderRadius: 12, padding: "0.65rem" }}>
        <Icon size={18} style={{ color: accent, opacity: loading ? 0.35 : 1 }} />
      </div>
    </div>
  </div>
);
  ...
  const skeleton = (
    <div style={{ minHeight: "100vh", padding: "1.25rem", background: "#18181b", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .dash-panels { display: grid; grid-template-columns: 3fr 2fr; gap: 1.5rem; }
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "1.25rem", background: "#18181b", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <h1 style={{ fontSize:"1.75rem", fontWeight:900, color:"#fafafa", letterSpacing:"-0.02em" }}>Admin Dashboard</h1>
