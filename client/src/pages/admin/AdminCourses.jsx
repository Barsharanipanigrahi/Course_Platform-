  ...
  <tr style={{
    ...
    <td style={{ padding: "1rem" }}>
      <div style={{ display: "flex", justifyContent: "center", gap: 20 }}>
        {[0, 1, 2].map(j => <div key={j} style={{ ...skeletonBase, width: 18, height: 18, borderRadius: 4 }} />)}
      </div>
    </td>
  </tr>
);
...
const SkeletonTable = () => (
  <div style={{ borderRadius: 16, overflow: "hidden", background: "#27272a", border: "1px solid rgba(245,158,11,0.15)" }}>
...
/* ── Helper: normalize category _id from course ── */
const getCatId = (course) =>
  course.category
...
const AdminCourses = () => {
  ...
  const getCategoryObj = (course) => {
    const id = getCatId(course);
    if (!id) return null;
    return categories.find(c => c._id === id) || null;
  };

  /* ── FIXED filter: normalize both sides to string before comparing ── */
  const filteredCourses = activeCategory
    ? courses.filter(course => String(getCatId(course)) === String(activeCategory))
    : courses;
    ...
    <div style={{ minHeight: "100vh", padding: "2rem", background: "#18181b", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .filter-btn {
          ...
          font-weight: 700; border: 1px solid rgba(245,158,11,0.25);
        ...
        .filter-btn.active { background: #f59e0b; border-color: #f59e0b; color: #18181b; box-shadow: 0 4px 14px rgba(245,158,11,0.35); }
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: "1.5rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: "1.875rem", fontWeight: 900, color: "#fafafa", display: "flex", alignItems: "center", gap: 10 }}>
            <BookOpen size={26} style={{ color: "#f59e0b" }} /> Courses
          </h1>
          <p style={{ fontSize: "0.8rem", color: "#52525b", marginTop: 4 }}>
            {filteredCourses.length} of {courses.length} course{courses.length !== 1 ? "s" : ""}
            {activeCategory ? ` in selected category` : " total"}
          </p>
        </div>
        <button
          onClick={() => setAddOpen(true)}
          style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 10, background: "#f59e0b", color: "#18181b", fontWeight: 700, border: "none", cursor: "pointer", boxShadow: "0 4px 20px rgba(245,158,11,0.35)", fontSize: "0.9rem" }}
            ...
            return (
              ...
              >
                <span style={{
                  ...
                  background: isActive ? "#18181b" : (cat.color || "#f59e0b"),
      ...
      {loading ? <SkeletonTable /> : (
        <div style={{ borderRadius: 16, overflow: "hidden", background: "#27272a", border: "1px solid rgba(245,158,11,0.15)", boxShadow: "0 8px 40px rgba(0,0,0,0.4)" }}>
          <table style={{ width: "100%", fontSize: "0.875rem", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "rgba(245,158,11,0.08)", borderBottom: "1px solid rgba(245,158,11,0.2)" }}>
                {["Title", "Description", "Category","duration", "Price", "Actions"].map((h) => (
              ...
              {filteredCourses.length === 0 ? (
                <tr>
                  <td colSpan="6" style={{ textAlign: "center", padding: "3rem", color: "#52525b", fontSize: "0.9rem" }}>
                    No courses found{activeCategory ? " for this category" : ""}
                  </td>
                </tr>
              ) : filteredCourses.map((course, i) => {
                const cat = getCategoryObj(course);
                return (
                  <tr key={course._id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)", transition: "background 0.15s" }}
                  ...
                  >
                    ...
                    {/*Duration POSITION */}
                    <td style={{ padding: "1rem", color: "#a78bfa", fontWeight: 700 }}>
                      {course.duration || "—"}
                    </td>
                    ...
                    <td style={{
                      padding: "1rem",
                      fontWeight: 700,
                      color: course.price === 0 ? "#22c55e" : "#f59e0b"
                    }}>
