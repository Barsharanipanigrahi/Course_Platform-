...
const SkeletonUserRow = ({ i }) => (
  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)" }}>
    ...
    <td style={{ padding: "1rem" }}><div style={{ ...skeletonBase, width: 52, height: 22, borderRadius: 100 }} /></td>
...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await api.get("/user/get", { headers: { Authorization: `Bearer ${token}` } });
  ...
  const deleteUser = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await api.delete(`/user/delete/${selected._id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const openViewModal = async (user) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await api.get(`/enrollment/user/${user._id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "1.25rem", background: "#18181b", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ marginBottom: "1.5rem", display: "flex", flexWrap: "wrap", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        <div>
          <p style={{ fontSize: "0.68rem", fontWeight: 700, color: "#f59e0b", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 4 }}>Management</p>
          <h1 style={{ fontSize: "1.75rem", fontWeight: 900, color: "#fafafa", display: "flex", alignItems: "center", gap: 10 }}>
            <Users size={24} style={{ color: "#f59e0b" }} /> Users
          </h1>
          <p style={{ fontSize: "0.8rem", color: "#52525b", marginTop: 4 }}>{users.length} user{users.length !== 1 ? "s" : ""} total</p>
        </div>
      </div>

      {/* Table */}
      <div className="table-scroll">
        <div style={{ background: "#27272a", minWidth: 480 }}>
          <table style={{ width: "100%", fontSize: "0.875rem", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="4" style={{ textAlign: "center", padding: "3rem", color: "#52525b", fontSize: "0.9rem" }}>No users found</td></tr>
              ) : (
                users.map((user, i) => (
                  <tr key={user._id}
                  ...
                  >
                    ...
                    <td style={{ padding: "1rem", color: "#a3a3a3", fontSize: "0.8rem" }}>{user.email}</td>
                    <td style={{ padding: "1rem" }}>
                      <span style={{ fontSize: "0.65rem", fontWeight: 700, padding: "3px 10px", borderRadius: 100, background: user.role === "admin" ? "rgba(245,158,11,0.15)" : "rgba(52,211,153,0.12)", color: user.role === "admin" ? "#f59e0b" : "#34d399" }}>
                        {user.role}
                      </span>
                    </td>
                    <td style={{ padding: "1rem" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                        <button className="action-btn" title="View Enrollments" onClick={() => openViewModal(user)} style={{ color: "#60a5fa", background: "rgba(96,165,250,0.12)", border: "1px solid rgba(96,165,250,0.25)" }}>
                          <Eye size={16} />
                        </button>
                        <button className="action-btn" title="Delete User" onClick={() => { setSelected(user); setDeleteOpen(true); }} style={{ color: "#f87171" }}>
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
      ...
      <ViewEnrollmentsModal open={viewOpen} onClose={() => { setViewOpen(false); setSelected(null); setEnrollments([]); }} courses={enrollments} loading={loadingEnrollments} userName={selected?.name} />
      <DeleteConfirmModal open={deleteOpen} onClose={() => { setDeleteOpen(false); setSelected(null); }} onDelete={deleteUser} title="Delete User?" description={`Are you sure you want to delete "${selected?.name}"? This action cannot be undone.`} confirmText="Delete User" />
    </div>
  );
};
